...
def initialize_weights(m: ResNet50) -> None:
...
class ModelRepository:
    ...
    def save(self, model: ResNet50, config: ModelConfig) -> None:
        self.save_weights(model, self._model_fp)
        self.save_config(config.__dict__, self._conf_fp)

    def load_model(self, model: ResNet50) -> ResNet50:
...
class ModelFactory:
    ...
    @staticmethod
    def load(repository: ModelRepository) -> t.Tuple[ResNet50, ModelConfig]:
...
def main() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("ResNet50_model.log"),
