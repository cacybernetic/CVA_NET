...
class Score:
    ...
    def compute(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        if self._average == 'binary':
            ...
            false_positive = ((y_true != self._pos_label) & (y_pred == self._pos_label)).sum().float()

            denominator = true_positive + false_positive
            if denominator == 0:
                precision = torch.tensor(self._zero_division, dtype=torch.float32, device=device)
            else:
            ...
            for cls in classes:
                ...
                denominator = true_positive + false_positive
                if denominator == 0:
                    precision_cls = torch.tensor(self._zero_division, dtype=torch.float32, device=device)
                else:
            ...
            if self._average == 'micro':
                ...
                total_false_positive = sum([((y_true != cls) & (y_pred == cls)).sum().float() for cls in classes])
