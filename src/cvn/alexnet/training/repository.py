...
from yolo.v3 import repository as yolo_repos
from yolo.v3.training.factory import yolo_trainer
from yolo.v3.training.optimizer import repository as optimizer_repos
from yolo.v3.training.optimizer.lr_scheduler import repository as scheduler_repos
...
def save_config(config: Config, dir_path: str, encoding: str='utf-8') -> Dict[str, Any]:
    ...
    yolo_model_dir = os.path.join(dir_path, 'model')
    ...
    # Save  model config;
    results['model_config'] = yolo_repos.save_config(config.model, yolo_model_dir, encoding)  # noqa
    # Save optimizer;
...
def save_data(trainer: YOLOTrainer, dir_path: str, device_type: str=None, encoding: str='utf-8') -> Dict[str, Any]:
    ...
    os.makedirs(dir_path, exist_ok=True)
    # Save yolo model;
    results['model'] = yolo_repos.save_data(model, yolo_model_dir, device_type=device_type)  # noqa
    # Save optimizer;
...
def load_config(dir_path: str, encoding: str='utf-8') -> Config:
    ...
    model_config = yolo_repos.load_config(yolo_model_dir, encoding)
...
def load_data(dir_path: str, config: Config, encoding: str='utf-8', trainer: YOLOTrainer=None) -> YOLOTrainer:
    assert dir_path, (
    ...
    # Load yolo model;
    trainer.model = yolo_repos.load_data(yolo_model_dir, config.model, trainer.model)
