...
class MaskCollator(object):
    ...
    def __call__(self, batch: int=100):
        '''
...
class MultiViewTransform:
    ...
    def __init__(self, size=224, patch_size: int=16, mask_value: float=0.0):
    ...
    def __call__(self, x: Image.Image, mask_id: int=0) -> Tuple[torch.Tensor, torch.Tensor]:
        x = self.transform(x)
        masks_enc, masks_pred = self.mask_collator(50)
        # print(len(masks_pred))
        masks_enc = masks_enc[0]
        masks_pred = masks_pred[0]
        # print(masks_enc.shape, masks_pred.shape)
        masks_enc = masks_enc[mask_id]
        masks_pred = masks_pred[mask_id]
        y1 = apply_mask_to_image(x, mask_indices=masks_enc, patch_size=self._patch_size, mask_value=self._mask_value)
        y2 = apply_mask_to_image(x, mask_indices=masks_pred, patch_size=self._patch_size, mask_value=self._mask_value)
...
class CustomImageDataset(BaseDataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        images = self._transform(image,  select)
        return images
