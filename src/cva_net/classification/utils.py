...
class ModelPredictionRender:
    ...
    def __init__(
        self, model, dataset, class_names, image_size, num_batchs=None,
        batch_size=None,
        num_rows=None, num_cols=None, fig_size=None, output_dir=None
    ) -> None:
        has_get_item_func = (
    ...
    def _predict(self, image):
        ...
        predictions, confidences = output_function(logits)
        self._times.append(time() - start_time)
        return predictions[0], confidences[0]

    def make_prediction(self, show_result=False):
        dataset_len = len(self._dataset)
        plt.figure()
        fig, axs = plt.subplots(
            self._num_rows, self._num_cols, figsize=self._fig_size
        )
        ...
        for i in range(self._num_rows):
            for j in range(self._num_cols):
                image_index = random.randrange(dataset_len)
        ...
        fig.suptitle(
            (
                f"Random {self._num_rows * self._num_cols} images taken"
                " from dataset with their predictions."
            ),
    ...
    def make_predictions(self, show_result=False):
        count = self._num_rows * self._num_cols

        for _ in range(self._num_batchs):
            self.make_prediction(show_result)

        self._pbar.write(
            f"Prediction of {self._num_batchs} batchs "
            f"of {count} samples are done."
        )
        self._pbar.close()
